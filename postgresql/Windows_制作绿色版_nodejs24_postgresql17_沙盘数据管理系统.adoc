...
[source,console]
----
mysql -h 127.0.0.1 -P 3307 -u root -pmypass -e "CREATE DATABASE data_management;"
...
这份文档整理出一套完整的 **绿色版 Node.js + PostgreSQL + WinSW** 部署方案。你可以根据这份文档直接配置并打包发给客户。
...
:: 1. 初始化数据库 (仅在首次安装时执行，PG_VERSION 是个文件)
