...
为纪念惠普机械键盘：定制化网址: https://hub.hpgthub.com/
...
* **阈值 (Threshold)**: 系统设定 `HEARTBEAT_THRESHOLD = 300s`（5分钟）。若两次操作间隔超过此值，系统认为用户进入了“非工作状态”。
...
== 5. 核心今日专注时长代码设计
[mermaid, target=img/mermaid-2026-05-09-113208, format=svg]
....
sequenceDiagram
    participant User as 👤 用户(键盘/鼠标)
    participant Backend as 🔧 key_mouse.py (后端)
    participant JSON as 📄 2026-05-09.json (磁盘)
    participant Frontend as 🖥️ index.html (前端)

    rect rgb(70, 130, 180)
        Note over User,Backend: === 后端写入链路 (每次按键/鼠标事件) ===

        loop 每次键盘按键 / 鼠标操作
            User->>Backend: log_event(is_keyboard, key_name)
            Backend->>Backend: now_ts = time.time()
            Backend->>Backend: 计算 focus_seconds 累加

            Backend->>Backend: daily_data["last_event_timestamp"] = now_ts<br/>写入内存

            Note over Backend: 防抖等待 (数秒~数十秒)
            Backend->>JSON: _atomic_save(daily_data)<br/>① 先写 .tmp<br/>② os.replace() 原子替换<br/>持久化到磁盘
        end
    end

    rect rgb(205, 133, 63)
        Note over Frontend,JSON: === 前端读取链路 ===

        Frontend->>Frontend: onMounted() 启动

        par 定时轮询 (每60秒)
            loop 每 60s
                Frontend->>JSON: fetch("key_stats/2026-05-09.json?t=timestamp")
                JSON-->>Frontend: 返回完整 JSON 数据
                Frontend->>Frontend: if date === 今天则更新 lastEventTimestamp.value
            end
        and 实时心跳 (每1秒)
            loop 每 1s (永不停止)
                Frontend->>Frontend: now.value = Date.now() / 1000
                Frontend->>Frontend: liveFocusSeconds computed 重算<br/>每秒读 lastEventTimestamp
                Frontend->>Frontend: isDormant computed 重算<br/>每秒读 lastEventTimestamp
                Frontend->>Frontend: watch(isDormant) 切换背景色<br/>页面显示实时专注时长
            end
        end
    end

    rect rgb(60, 179, 113)
        Note over User,Frontend: === 典型场景示例 ===

        User->>Backend: T=10:00:00 按下 A 键
        Backend->>JSON: 写入 last_event_timestamp = 1715265600.0

        Note over Frontend: T=10:01:00 轮询触发 fetch
        Frontend-->>Frontend: 更新 lastEventTimestamp.value

        Note over Frontend: T=10:00:01 ~ 10:04:59<br/>每秒 liveFocusSeconds +1

        Note over Frontend: T=10:05:00 delta >= 300<br/>isDormant=true 背景变灰
    end
....
