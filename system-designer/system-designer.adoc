...
[horizontal]
V (View) :: 展示端：组件 (+page.svelte/*.vue)。像“顾客”，只负责绑定数据和触发事件。它是“愚笨”的，不关心后端细节。

VM (ViewModel) :: 状态管理 (store.svelte.ts/composables函数)。像“大堂经理”，持有 `$state`，封装前端处理逻辑，并通过 fetch 调用  API

M (Model) :: 获取数据：数据源代理 (operations.ts)。像“快递员”，定义 TS 接口，负责发起真正的 fetch/axios 请求。
...
* **JavaScript (JS)：**
    ...
    ** 网络传输效率：为了跨平台和网络分发而采用##源码传输##。编译后的二进制文件往往比压缩后的文本（Gzip/Brotli）大得多。在 Web 早期，带宽极其珍贵，传输文本更高效。
    ** 安全性： 浏览器是一个高度受限的“沙箱”。如果执行预编译的代码，浏览器很难实时监控代码是否在非法操作内存，安全性会大打折扣。
    ** 当前的“中间方案”： 实际上，现代浏览器已经进化了。它们不再是简单的“逐行解释”，而是采用了 JIT (Just-In-Time) 编译技术。Chrome 浏览器的现代引擎（V8）使用 **JIT (Just-In-Time)** 在运行时将热点代码转为机器码。这样代码的执行效率更高。
+
**小结：**现在的做法是： JS 代码以文本形式传给浏览器，浏览器在运行过程中，使用 JIT 技术把经常执行的热点代码直接编译成机器码。这兼顾了文本传输的灵活性和编译语言的高性能。
...
* **优势**：
    ...
    ** **前端生态融合**：与 React (R3F) 或 Svelte (Threlte) 或 Vue3(TresJS) 完美结合。
