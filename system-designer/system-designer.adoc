...
* **JavaScript (JS)：** 为了跨平台和网络分发而采用源码传输。现代引擎（V8）使用 **JIT (Just-In-Time)** 在运行时将热点代码转为机器码。
...
1. 代码写 js 文件，不写复杂的类型标。
2. 配置 jsconfig or tsconfig （tsconfig.json 应该会被优先读取）
