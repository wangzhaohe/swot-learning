...
==== 9.4.1 1. LOAD DATA LOCAL INFILE -- 批量导入数据 -- 了解
文档网址: https://dev.mysql.com/doc/refman/8.3/en/loading-tables.html

一般用于数据的初始化，这种方式比较麻烦，因为有一此条件：
...
==== 9.4.4 4. SELECT FROM WHERE -- 获取记录语法
# 语法格式：
#
...
#     indicates what you want to see. This can be a list of columns, or * to indicate “all columns.”  * 表示所有列
#
# which_table 哪个表::
...
==== 9.4.5 5. SELECT * FROM     -- 获取所有列
# .运行命令
# [source,python,]
# ----
#
# sql = "SELECT * FROM ContentCategory;"
# ----
#
# .运行结果
# ....
...
==== 9.4.6 6. SELECT * FROM WHERE  -- 条件选择
# 选择特定条件的行
#
# .运行命令
# [source,python,]
# ----
#
# sql = "SELECT * FROM ContentCategory WHERE title = '解决方案';"
# ----
#
# .运行结果
# ....
...
# ----
#
# sql = "SELECT * FROM ContentCategory WHERE createDate >= '2024-02-01';"
# ----
#
# .运行结果
# ....
...
==== 9.4.7 7. SELECT * FROM WHERE AND/OR -- 条件选择
# .运行命令
# [source,python,]
# ----
#
# sql = '''SELECT * FROM ContentCategory WHERE useFlag='Y' AND createDate >= '2024-01-01';'''
# ----
#
# .运行结果
# ....
...
# ----
#
# sql = '''SELECT * FROM ContentCategory WHERE useFlag='N' OR createDate <= '2024-01-31';'''
# ----
#
# .运行结果
# ....
...
==== 9.4.8 8. SELECT column FROM   -- 选择指定列
...
# ----
#
# sql = "SELECT title, description FROM ContentCategory;"
# ----
#
# .运行结果
# ....
...
# ----
#
# sql = '''SELECT owner FROM ContentCategory;
...
==== 9.4.9 9. SELECT FROM ORDER BY -- 排序 重要内容
# 对行进行排序
#
# .运行命令
# [source,python,]
# ----
#
# sql = """SELECT title, description, createDate 
...
# ----
#
# sql = """SELECT title, useFlag, createDate 
#          FROM ContentCategory ORDER BY useFlag, createDate DESC;"""
# ----

==== 9.4.10 10. SELECT * FROM LIMIT  -- 分页 重要内容
# LIMIT 子句常用于计算分页。在网页中的分页设计中，通常浏览器传递两个参数来控制分页：
#
# 1. **页码（Page Number）**：表示当前所在的页码。
# 2. **每页显示数量（Page Size）**：表示每页显示的记录数。
#
# 在 SQL 查询中，通过 `LIMIT` 子句来实现分页。`LIMIT` 子句包括两个参数，分别是起始位置和要返回的行数。通常，起始位置可以通过计算 `(页码 - 1) * 每页显示数量` 得到，而要返回的行数即为每页显示数量。
#
# 因此，在网页中的分页设计中，你可以计算出起始位置和每页显示数量，然后将其应用到 SQL 查询中。例如，如果你希望显示第 3 页，每页显示 10 条记录，那么起始位置为 `(3 - 1) * 10 = 20`，要返回的行数为 10，你可以构造如下的 SQL 查询：
#
# ```sql
...
==== 9.4.11 11. SELECT UNION ALL SELECT  -- 不去除重复行 了解内容
...
===== 9.4.11.4 union all
我们想要将这两个表格的数据合并成一个结果集，可以使用 `UNION ALL` 操作符。
...
# ----
#
# sql = "SELECT title, createDate, MONTH(createDate) FROM ContentCategory;"
# ----
#
# .运行结果
# ....
...
# ----
#
# sql = """SELECT MONTH(DATE_ADD(CURDATE(), INTERVAL 1 MONTH));"""
# sql = """SELECT MOD(MONTH(CURDATE()), 12) + 1;"""
# ----
#
# 可以先查看上面函数执行的结果为：
#
...
# ----
#
# sql = """SELECT title, createDate FROM ContentCategory
...
====== 9.4.12.4.1 MOD 函数解释
这段 SQL 代码的目的是计算**下一个月份**，无论当前的月份是多少，它都会返回当前月份的下一个月。
...
### 结果：
- 如果当前是 9 月，这个查询会返回 **10**。
- 如果当前是 12 月，这个查询会返回 **1**。

### 总结：
这段 SQL 代码是用来计算当前月份的下一个月，并且在 12 月后返回 1 月。
...
sql = """SELECT 1 IS NULL, 1 IS NOT NULL;"""
# ----
#
# .运行结果
# ....
...
# ----
#
# sql = """SELECT 1 = NULL, 1 <> NULL, 1 < NULL, 1 > NULL;"""
# ----
#
# .运行结果
# ....
...
==== 9.4.14 14. MySQL 自带模式匹配语法 Pattern Matching
...
===== 9.4.14.1 举例查找 owner 字段以 S 开头的记录
# .运行命令
# [source,python,]
# ----
#
# sql = """SELECT title, owner FROM ContentCategory
#          WHERE owner LIKE 'S%'
# ;"""
# ----
#
...
# .运行命令 -- COLLATE utf8mb4_bin 表示区分大小写
# [source,python,]
# ----
#
# sql = """SELECT title, owner FROM ContentCategory
...
# ....
#
# NOTE: 将 LIKE 'S%' 换成 LIKE 's%'，则会查询匹配小写字母 s 的记录。
...
==== 9.4.15 15. MySQL 扩展正则表达式语法 Regular Expression
# https://dev.mysql.com/doc/refman/8.3/en/pattern-matching.html
#
...
# ----
#
# sql = """SELECT COUNT(*) FROM ContentCategory;"""
# ----
#
# .运行结果: 总行数为 8
# ....
...
# ----
#
# sql = """SELECT owner, COUNT(*) FROM ContentCategory GROUP BY owner;"""
# ----
#
# .运行结果
# ....
...
==== 9.4.19 19. UPDATE SET WHERE  -- 更新记录
# 只更新需要更新的记录。
#
# .运行命令
# [source,python,]
...
# ----
#
# sql = """DELETE FROM ContentCategory;"""
# ----
#
# WARNING: 谨慎操作!!!
#
...
# ----
#
# sql = """CREATE USER 'zhangsan'@localhost
#          IDENTIFIED BY 'Password1=';
# """
# ----
#
# IMPORTANT: 只能在 MySQL 服务器安装的本地机器上登录。
#
#
# ***
# 创建能在任意 IP 上登录的用户
...
# ----
#
# sql = """revoke delete on OfficialWebsite.* from 'lisi'@'%';"""
# sql = """revoke all on OfficialWebsite.* from 'lisi'@'%';"""   删除 lisi 的所有权限
# ----
#
#
...
# ----
#
# sql = """show create TABLE ContentInfo;"""
# ----
#
# .运行结果
# ....
...
# ```
#
# 这个查询将从 `User` 表和 `UserDetails` 表中检索数据，并将它们连接起来以获取完整的用户信息。

=== 9.8 外键关联多对多


==== 9.8.1 1. 多对多关系举例
# 一个常见的多对多关系的例子是学生和课程之间的关系。*一个学生可以选修多门课程，而一门课程也可以被多名学生选修。*
#
...
# ```
#
# 在关联表中，`student_id` 和 `course_id` 列分别引用学生表和课程表中的主键，表示学生选修了哪些课程，而每一行表示一个学生选修了一门课程的记录。
#
...
# ```
#
# 在关联表 `StudentCourses` 中，`student_id` 和 `course_id` 列分别引用 `Students` 表和 `Courses` 表中的主键，并且还定义了一个复合主键，以确保每个学生只能选修一门课程一次。
...
# +
# ```sql
# SELECT Courses.name AS course_name, StudentCourses.grade
# FROM Students
# JOIN StudentCourses ON Students.student_id = StudentCourses.student_id
# JOIN Courses ON StudentCourses.course_id = Courses.course_id
...
#
# 在 MySQL 中，你可以通过在表定义中指定主键来创建主键约束。主键可以包含一个或多个列，但每个表只能有一个主键。主键约束提供了以下几点特性：
#
# 1. **唯一性**：主键列的值在整个表中必须是唯一的，即每行数据都必须具有唯一的主键值。
#
# 2. **非空性**：主键列的值不能为 NULL，因为主键用于唯一标识每一行数据。
#
# 3. **索引**：主键列会自动创建索引，这样可以加快数据检索的速度。
#
# 在创建表时指定主键约束的一般语法如下：
#
# ```sql
...
# ```
#
# 在上面的语法中，`column1` 是主键列，`datatype` 是该列的数据类型。你还可以在 `PRIMARY KEY` 关键字后面指定多个列，以创建复合主键。例如：
#
# ```sql
# CREATE TABLE table_name (
#     column1 datatype,
...
# ```
# ....
# 如果 id 字段被别的表做为外键使用，请先删除该外键。
# 假设被 ContentInfo 使用了，且名称为`ContentInfo_ibfk_1`，要删除名为 `ContentInfo_ibfk_1` 的外键约束，您可以使用以下 SQL 语句：
#
# 这条语句将从 `ContentInfo` 表中删除名为 `ContentInfo_ibfk_1` 的外键约束。
# ....
#
#
# .删除 id 自增
...
3. 发送到邮箱 27129782.qq.com

IMPORTANT: 不要忘记在邮件中写上自己的名字。
