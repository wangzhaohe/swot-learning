...
- Node.js 的全局变量包括 __filename 和 __dirname 等
...
// 严格地说，应是当前模块的文件名，即当前的模块文件的对路径(符号链接会被解析)。对主程序来说，这不一定与命令行中使用的文件名相同。

`__dirname`  指向当前运行的脚本所在的目录。
// 严格地说，指向的应是当前模块的目录名，与 path.dirname(__filename) 返回的路径相同。
