...
----
+
_问题_：如果要跑100圈或者10000圈呢？复制粘贴会累死人，而且代码极其丑陋。
...
* *continue*：跳过本次循环，直接进入下一次。
  ** _场景_：吃苹果，发现第3个烂了，扔掉，继续吃第4个。

* *break*：直接结束整个循环。
  ** _场景_：吃苹果，吃到第3个发现里面有半条虫子，恶心得剩下的全不吃了。
...
[plantuml, target=img/diagram-name, format=svg]
....
@startuml
...
function generateGoods(count) {
    ...
    for (let i = 1; i <= count; i++) {
        ...
        let goods = {
            ...
            date: "2026-10-" + (Math.floor(Math.random() * 28) + 1), // 随机日期
...
* 主要将 `generateGoods` 函数进行升级，让它再调用写好的一些工具函数。
...
*思路*：遍历数组，如果发现元素等于20，就 `splice` 删除它。 +
* 注意：删除后索引要回退一步。
...
=== 三：操作元素内容
下表总结了操作 DOM 元素内容时最常用的两个属性：

[cols="1,2,3", options="header"]
|===
| 属性 | 功能描述 | 特点
| `innerText` | 获取或设置标签内的 *纯文本* | 不识别 HTML 标签，读取时会触发重排（Reflow），自动过滤隐藏元素的文本。
| `innerHTML` | 获取或设置标签内的 *HTML 代码* | 识别并解析标签，保留原始格式。*开发中最常用，但需注意安全*。
|===
...
<body>
    ...
    <script>
        ...
        // 1. 读取测试
        console.log('innerText:', box.innerText);   // 输出: "你好" (粗体没了，隐藏内容也没了)
        ...
        const peach = "<img src='x' onerror='alert(\"XSS攻击!\")'>";
        box.innerHTML = peach; 
    </script>
</body>
</html>
...
[CAUTION]
====
* 永远不要将未经脱敏的用户输入直接赋值给 `innerHTML`，这会导致跨站脚本攻击（XSS）。
* 对于纯文本展示，建议优先使用 `textContent` 或 `innerText`。
====
