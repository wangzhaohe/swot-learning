...
|===
| 场景 | 文件名要求 | 示例
| 包含 `public class A` | **必须**叫 `A.java` | 强制一致
| 包含 `class B` (非 public) | 可以叫 `B.java` 或 `Custom.java` | 灵活命名
| 包含 `public class A` 和 `class B` | **必须**叫 `A.java` | 以 public 类为准
|===
...
public class VariableDemo {
    public static void main(String[] args) {
        ...
        int swipeCount = 0;          // 今日刷卡次数
        boolean isActive = true;     // 卡片是否挂失？true代表正常
...
. 为什么要学？
+
如果每天有 1000 个人刷卡，难道要写 1000 次校验卡号格式的 `if/else` 吗？
+
...
+
记住一句话：你不能刷“图纸”进门，你只能刷装在墙上的“门禁机实体” 。在 Java 中，我们要先写类，再用 new 关键字制造出对象 。
...
// 2. 使用类（安装并使用具体的门禁机）
public class TestDoorSystem {
    public static void main(String[] args) {
        // 使用 new 关键字，根据图纸造出一台真实的门禁机
        DoorMachine southGate = new DoorMachine();
        ...
        DoorMachine libraryGate = new DoorMachine();
...
public class StaticDemo {
    public static void main(String[] args) {
        // 1. 造两台具体的门禁机
        DoorMachine southDoor = new DoorMachine("南大门");
        DoorMachine northDoor = new DoorMachine("北大门");
        ...
        System.out.println("今日全校累计刷卡总数：" + DoorMachine.totalSwipes); 
    }
}
----
 ...
 */
class User {
    // 1. 私有化属性：像芯片数据一样锁起来，外部无法直接用 .获取
    private String id;        // 工号 (绝对核心，不可修改)
    private String name;      // 姓名 (允许修改，但需校验)
    private String cardId;    // 门禁卡号 (允许换卡)
    private boolean isActive; // 账户是否激活 (允许挂失/解挂)

    // 2. 无参数构造方法：Java 编程习惯
    public User() {}

    // 3. 全参数构造方法：发卡时一键初始化信息
    ...
    // ================== 核心防线 ==================
    // 4. 工号的 Getter：只提供“查看”窗口
    public String getId() {
        return id;
    }
    // 【关键点】故意不提供 setId() 方法！
    // 外部世界永远无法修改工号，实现了真正的“只读”保护。
    // ==============================================

    // 5. 姓名等其他属性的 Getter 和 Setter（对外提供的“安全读写接口”）
    public String getName() {
        return name;
    }

    public void setName(String name) {
        // 封装的最大好处：可以在修改数据时加入安全校验逻辑
        if (name != null && !name.trim().isEmpty()) {
            this.name = name;
        } else {
            System.out.println("【警告】姓名不能为空，修改失败！");
        }
    }
