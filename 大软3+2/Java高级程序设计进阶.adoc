...
==== 直接安装开发环境 IntelliJ IDEA -- 略
idea 下载地址：
https://www.jetbrains.com/idea/download/

在 Idea 创建新项目时可以直接安装 java jdk17。
...
==== IDEA 使用 podman JDK17 开发测试
本手册指导如何在 macOS 上，利用 Podman 容器作为 JDK 17 环境进行 Java 基础开发，实现本地「零 JDK 安装」。

===== 创建基础 Java 项目
1. 点击 **New Project**。
2. **Name**: `JavaBasics`
3. **Language**: `Java`
4. **Build system**: `IntelliJ` (⚠️ 务必选此项，不使用 Maven/Gradle)
5. **JDK**: 此时下拉框选 `No SDK`。
6. 点击 **Create**。

===== 关联容器 JDK (解决智能提示)
此步骤将 Podman 镜像内的 JDK 类库索引到本地 IDEA，消除代码红线。

1. 进入 **File** -> **Project Structure** (`Cmd + ;`) -> **SDKs**。
2. 点击 **+** -> **Add JDK...** -> **On Target...**。
3. 选择 **Docker** 目标：
   * **Server**: 选择刚才配好的 `Podman`。
   * **Image**: 选择 `eclipse-temurin:17-jdk`。
4. 点击 **Next**，IDEA 会启动容器并扫描 JDK 核心模块（如 `java.base`）。
5. 扫描完成后，在 **Project** 选项卡中，确保 **Project SDK** 选中该容器 JDK。

===== 配置运行目标 (Run on Podman)
确保代码实际在容器内执行，而非本地。
...
public class Main {
    public static void main(String[] args) {
        System.out.println("运行环境 OS: " + System.getProperty("os.name"));
        System.out.println("JDK 运行版本: " + System.getProperty("java.version"));
    }
}
----

1. 点击代码左侧的绿色运行箭头，选择 **Modify Run Configuration...**。
2. 找到 **Run on** 下拉菜单，点击 **New Target...** -> **Docker**。
3. 配置参数：
   * **Server**: `Podman`
...
[NOTE]
====
由于操作系统显示为 `Linux`，证明代码已成功脱离 macOS 本地环境，运行在 Podman 容器中。
====
...
[source,console]
----
run --rm -i -v $FileDir$:/src -w /src eclipse-temurin:17-jdk sh -c "javac *.java && java $FileNameWithoutExtension$"
----
