...
Create a hard link:
    ...
    ln ~/.rvm/gems/ruby-2.7.2/gems/asciidoctor-2.0.23/data/stylesheets/asciidoctor-default.css asciidoctor/
    Now I can edit asciidoctor-default.css in directory asciidoctor.
...
----
# 这是直接指定要使用画图
# command = f"{prog} {i_path} -o {o_path} -b html5 -r asciidoctor-diagram"
# 这是使用 options.rb 配置文件
...
[mermaid, target=img/mermaid-2025-05-09-113732, format=svg]
....
graph TD;
    A-->B;
    A-->C;
...
== github 条件判断渲染图片
在 github 上不会渲染 `mermaid`，所以只能使用加载图片的方式 `image::`，下面是判断的代码
...
IMPORTANT: 实际写的时候没有 source，也没有转义的 `\`，这里有只是为了能以代码的形式显示出来。
