...
执行种子文件:
  ...
  pnpm run seed --specific 000_delete_tables.js  // 执行特定种子文件
  knex seed:run --specific 000_delete_tables.js  // 同上

==== @file data/seeds/100_roles.js
[source,typescript]
----
import bcrypt from 'bcryptjs';
...
==== 总结
- **只用 `<Can>` 隐藏内容体验不好，建议用 Nuxt3 中间件做路由级权限拦截。**
